...
db = client["Jurong-Users"]
col_account = db["Account"]
col_household = db["Household"]

class acc_Jurong(acc_hougang_pb2_grpc.acc_HougangServicer):
...
def serve():
    ...
    acc_hougang_pb2_grpc.add_acc_HougangServicer_to_server(acc_Jurong(),server)
    port = '50055'
    server.add_insecure_port('[::]:50055')
    server.start()
    print("Account service server for Jurong started, listening on " + port)
